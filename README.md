...
### Пост-настройка (Скрипт первого запуска)
В репозитории есть скрипт `scripts/first-run-setup.sh`. Он нужен для автоматизации ручных действий, которые мы выполняли на сервере:
1. **Настройка времени Raspberry Pi**: Устанавливает афинский часовой пояс (`Europe/Athens`) и включает синхронизацию времени.
2. **Патч AuthMe**: Настраивает плагин так, чтобы пускать игроков с телефонов без пароля (интеграция Floodgate), разрешает символ точки `.` в никнеймах и увеличивает таймаут авторизации до 120 секунд.

**Как использовать:**
Так как AuthMe генерирует свой конфиг только при первом включении, этот скрипт нужно запустить **один раз** после самого первого успешного старта сервера. Зайдите на сервер по SSH и выполните:
```bash
cd /opt/minecraft
bash scripts/first-run-setup.sh
```
